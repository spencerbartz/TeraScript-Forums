...
function checkFormFields(formToCheck, errorDiv, ignoreList)
{
	...
	console.log("Form ignore list :" + ignore.toString());
	if(ignoreList != null)
	...
	//Check to see if any textfields, password fields, or textareas are blank
	for(var i = 0; i < elmnts.length; i++)
	{
		console.log("ELEMENT TYPE: " + elmnts[i].type);
		if(elmnts[i].type == "text" || elmnts[i].type == "textarea" || elmnts[i].type == "password")
		{
		...
		//Check that the password and reenter password fields are equal
		if(elmnts[i].type == "password")
		{
			if(elmnts[i].name == "password")
			{
				password = elmnts[i].value;
				//elmnts[i].value = MD5(elmnts[i].value);
			}
			else if(elmnts[i].name == "reenter")
			{
				reenter = elmnts[i].value;
				//elmnts[i].value = MD5(elmnts[i].value);
			}
		}
	...
	if(password != reenter || (password == "" && reenter == ""))
	{
		...
		myDiv.innerHTML = "Error: Passwords do not match.";
		return false;
	}
